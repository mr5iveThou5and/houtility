...
def colorize_op(kwargs):
    ...
    try:
        ...
        if t == 'Sop/null':
            ...
            hou_parm_template = hou.FloatParmTemplate("centroid", "centroid", 3, default_value=([0, 0, 0]), min=-1, max=1, min_is_strict=False, max_is_strict=False, look=hou.parmLook.Regular, naming_scheme=hou.parmNamingScheme.XYZW)
            folder_geo_info.addParmTemplate(hou_parm_template)
            hou_parm_template = hou.FloatParmTemplate("bbox_size", "bbox_size", 3, default_value=([0, 0, 0]), min=-1, max=1, min_is_strict=False, max_is_strict=False, look=hou.parmLook.Regular, naming_scheme=hou.parmNamingScheme.XYZW)
            folder_geo_info.addParmTemplate(hou_parm_template)
